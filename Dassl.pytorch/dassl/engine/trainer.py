...
class TrainerBase:
    ...
    def sp_model_inference(self, input, sp_prompt):
        raise NotImplementedError
...
class SimpleTrainer(TrainerBase):
    ...
    def build_data_loader(self):
        ...
        self.train_loader_fgpt = dm.train_loader_fgpt  # [FVGPT_Dassl]
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            time_start = time.time()  # [Compute_Cost]
            ...
            # print("Inference FPS:", self.cfg.DATALOADER.TEST.BATCH_SIZE / time_sum)
    ...
    def run_epoch(self):
        ...
        for self.batch_idx, batch in enumerate(self.train_loader_x):
            ...
            if meet_freq or only_few_batches:
                ...
                info += [f"lr {self.get_current_lr():.4e}"]
                info += [f"eta {eta}"]
                info += [f"max mem {query_mem:.1f}"]  # [TEST] 显存占用
                print(" ".join(info))
...
# [FVGPT_Dassl] Build a Trainer that can pass mask view (mainly build "train_loader_fgpt")
class TrainerFGPT(SimpleTrainer):
    """A base trainer using labeled data only."""
    ...
    def run_epoch(self):
        ...
        batch_time = AverageMeter()
        data_time = AverageMeter()
        self.num_batches = len(self.train_loader_fgpt)  # [FVGPT_Dassl]

        end = time.time()
        for self.batch_idx, batch in enumerate(self.train_loader_fgpt):  # [FVGPT_Dassl]
            data_time.update(time.time() - end)
            loss_summary = self.forward_backward(batch)
            batch_time.update(time.time() - end)
            losses.update(loss_summary)
