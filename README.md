...
> Haoyang Li<sup>1,2</sup>, Liang Wang<sup>1,2</sup>, Siyu Zhou<sup>1,3</sup>, Jiacheng Sun<sup>2</sup>, Jing Jiang<sup>1</sup>, Chao Wang<sup>2</sup>, Guodong Long<sup>1</sup> and Yan Peng<sup>2</sup>. <br>
> _<sup>1</sup>Australian Artificial Intelligence Institute &emsp; <sup>2</sup>Shanghai University &emsp; <sup>3</sup>Shanghai Artificial Intelligence Laboratory_     <br>
...
2. Prepare the dataset. We release 11 prompt-tuning datasets _with foreground views_ on [[🤗HuggingFace](https://huggingface.co/datasets/JREion/Prompt_Tuning_Datasets_with_Foreground)], just use them directly.
   ...
   <br> Details of data preparation can be found in [DATASETS.md](https://github.com/JREion/FVG-PT/blob/main/docs/DATASETS.md).
3. Run fine-tuning script on the backbone models first (e.g., CoOp): <br>
   ```bash
   ...
   ```
4. Run FVG-PT fine-tuning based on the pre-tuned backbone models: <br>
    ```bash
