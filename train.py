...
def extend_cfg(cfg):
    ...
    cfg.TRAINER.COOP.ALPHA = 1.0  # [AmPT_KgCoOp] for KgCoOp but NOT USE
    cfg.TRAINER.COOP.W = 2.0  # [AmPT_KgCoOp] for KgCoOp
    ...
    cfg.TRAINER.AMPT.EMA_MOMENTUM = 0.999
    cfg.TRAINER.AMPT.DISTILL_T = 2.0
    cfg.TRAINER.AMPT.MASK_THRESH = 0.5
    cfg.TRAINER.AMPT.DCA_SCALE_BY_REL = True

    ''' === [FVG-PT] CONFIGS === '''
    cfg.TRAINER.AMPT.RDBOOST_BOTTLENECK = 64
    cfg.TRAINER.AMPT.RDBOOST_RG_HIDDEN = 32
    cfg.TRAINER.AMPT.RDBOOST_MASK_THRESH = 0.5
    cfg.TRAINER.AMPT.RDBOOST_LOGIT_SCALE = 100.0
    # ---------- Base branch (FRG+FDC) ----------
    cfg.TRAINER.AMPT.RDBOOST_LAMBDA_D = 10.0   # Training weight for base branch
    # ------------- New branch (PC) -------------
    cfg.TRAINER.AMPT.BDPC_LAMBDA = 1.0  # Training weight for new branch (base-only training, but PC used only for new inference)
    cfg.TRAINER.AMPT.BDPC_USE_KL = True  # Optional KL to keep PC close to CLIP prior on base data (helps transfer robustness)
    # ------ Other fixed or discard params ------
    cfg.TRAINER.AMPT.RDBOOST_TAU_D = 2.0   # Tempeture. All RDBOOST_TAU_XXX is suggested to be the same.
    cfg.TRAINER.AMPT.BDPC_TAU_PC = 2.0
    cfg.TRAINER.AMPT.BDPC_TAU_KL = 2.0
    cfg.TRAINER.AMPT.RDBOOST_DELTA = 0.0
    cfg.TRAINER.AMPT.RDBOOST_LAMBDA_RG = 1.0
    cfg.TRAINER.AMPT.RDBOOST_MASK_THRESH = 0.5
    cfg.TRAINER.AMPT.RDBOOST_AREA_MIN = 0.02
    cfg.TRAINER.AMPT.RDBOOST_AREA_MAX = 0.98
    cfg.TRAINER.AMPT.BDPC_ALPHA_MAX = 1.0  # [Discard]
    cfg.TRAINER.AMPT.BDPC_TAU_BG = 2.0  # [Discard]
    cfg.TRAINER.AMPT.BDPC_USE_BG_IF_MASK = True  # [Discard]
    cfg.TRAINER.AMPT.BDPC_USE_RG_IF_MASK = True
    cfg.TRAINER.AMPT.RDBOOST_FDC_INIT_SCALE = 1e-3
...
def setup_cfg(args):
    ...
    if cfg.TRAINER.NAME == "CoPrompt" or cfg.TRAINER.NAME == "FVGPT_CoPrompt":
